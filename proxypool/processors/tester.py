...
from proxypool.setting import PROXY_SCORE_MIN
...
class Tester(object):
    ...
    async def validator(self, proxy: Proxy):
        """
        test single proxy
            ...
            try:
                ...
                async with session.get(TEST_URL, proxy=f'http://{proxy.string()}', timeout=TEST_TIMEOUT,
                    ...
                    if response.status in TEST_VALID_STATUS:
                        ...
                        score = self.redis.decrease(proxy)
                        logger.debug(f'proxy {proxy.string()} is invalid, decrease score')
                        if score < PROXY_SCORE_MIN:
                            self.redis.rem(proxy)
            except EXCEPTIONS:
                score = self.redis.decrease(proxy)
                if score < PROXY_SCORE_MIN:
                    self.redis.rem(proxy)
    ...
    def run(self):
        ...
        while True:
            ...
            if proxies:
                tasks = [self.validator(proxy) for proxy in proxies]
...
def run_tester():
    host = '96.113.165.182'
    port = '3128'
    tasks = [tester.validator(Proxy(host=host, port=port))]

...
class RedisClient(object):
    ...
    def decrease(self, proxy: Proxy) -> int:
        ...
        :return: new score
        """
        proxy_str = f'http://{proxy.string()}' if not proxy.string().startswith('http') else proxy.string()
        if IS_REDIS_VERSION_2:
            self.db.zincrby(REDIS_KEY, proxy_str, -1)
        else:
            self.db.zincrby(REDIS_KEY, -1, proxy_str)
        score = self.db.zscore(REDIS_KEY, proxy_str)
        logger.info(f'{proxy.string()} score decrease 1, current {score}')
        # if score <= PROXY_SCORE_MIN:
        #     logger.info(f'{proxy.string()} current score {score}, remove')
        #     self.db.zrem(REDIS_KEY, proxy_str)
        return score

    def rem(self, proxy: Proxy):
        proxy_str = f'http://{proxy.string()}' if not proxy.string().startswith('http') else proxy.string()
        logger.info(f'{proxy.string()} current score 0, remove')
        self.db.zrem(REDIS_KEY, proxy_str)
    ...
    def exists(self, proxy: Proxy) -> bool:
        ...
        """
        proxy_str = f'http://{proxy.string()}' if not proxy.string().startswith('http') else proxy.string()
        return not self.db.zscore(REDIS_KEY, proxy_str) is None
    ...
    def max(self, proxy: Proxy) -> int:
        ...
        if IS_REDIS_VERSION_2:
            return self.db.zadd(REDIS_KEY, PROXY_SCORE_MAX, proxy_str)
        return self.db.zadd(REDIS_KEY, {proxy_str: PROXY_SCORE_MAX})
    ...
    def batch(self, cursor, count) -> tuple[Any, Any]:
        """
        ...
        proxies = (proxy[0].replace('http://', '') for proxy in proxies)
        return cursor, convert_proxy_or_proxies([i for i in proxies])
